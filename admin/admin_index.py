    ...
    def get(self):
        ...
        if not user:
            ...
            if user.admin:
                ...
                navbars = Navbar.all() \
                    .filter("author =", user.user) \
                    .order('name') \
                    .fetch(200)
                # Il voit tous les menus qu'il a crees
                # et tous ceux qui ont ete declares commun par un admin
                menus = Menu.all() \
                    .filter("author =", user.user) \
                    .order('name') \
                    .fetch(200)
            ...
            menus_with_navbars = map(
                lambda menu: {
                    'name': menu.name,
                    'special_kind': menu.special_kind,
                    'key': menu.key(),
                    'author': menu.author,
                    'shared': menu.shared,
            ...
            self.response.out.write(
                template.render(
                    path,
                    {'menus': menus_with_navbars,
                     'commun_menus': commun_menus_with_navbars,
                     'navbars': navbars,
                     'url': url,
